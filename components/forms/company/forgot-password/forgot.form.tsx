...
import { companyForgotSchema, CompanyForgotValues } from "./forgot.validation";
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";
import Image from "next/image";

export default function CompanyForgotForm() {
  const [success, setSuccess] = useState(false);

  const form = useForm<CompanyForgotValues>({
    resolver: yupResolver(companyForgotSchema),
    defaultValues: { email: "" },
  });

  const onSubmit = (values: CompanyForgotValues) => {
    ...
    return (
      <div className="text-center bg-white border border-gray-100 rounded-2xl p-10 shadow-md">
        ...
        <p className="text-gray-600 text-sm">
          Please check your company email for the OTP to reset your password.
        </p>
      </div>
    );
  }
